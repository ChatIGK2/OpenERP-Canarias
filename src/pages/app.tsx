...
import { ModuleType, Lead, HistoryEntry, LeadEmail, Meeting, Quotation, Order, Invoice, DDT, Product, StockMove, JournalEntry, EmailCampaign, Agent, DocCounters, Carrier } from '../types';
import { Language } from '../i18n';
import { initialLeads, initialHistory, initialLeadEmails, initialMeetings, initialQuotations, initialOrders, initialInvoices, initialDDTs, initialProducts, initialStockMoves, initialJournalEntries, initialEmailCampaigns, initialAgents, initialDocCounters, initialCarriers } from "../data/mock-data";
...
export default function App() {
  ...
  const handleLogin = (newSession: Session) => {
    ...
    if (validation.user) setCurrentUser(validation.user);
  };

  const handleLogout = () => {
    if (session) authService.logout(session.token);
    setSession(null);
    setCurrentUser(null);
    localStorage.removeItem('session');
  };

  // Navigazione dai bottoni del modulo Contacts
  const handleNavigateFromContacts = (page: string) => {
    const map: Record<string, ModuleType> = {
      'orders': 'orders',
      'invoicing': 'invoicing',
      'contacts': 'contacts'
    };
    if (map[page]) setActiveModule(map[page]);
  };
      ...
      case 'contacts':
        return (
          <Contacts
            ...
            language={language}
            onNavigate={handleNavigateFromContacts}
          />
        );
      case 'inventory':
